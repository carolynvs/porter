...
func (b *DockerBuilder) BuildInvocationImage(manifest *manifest.Manifest) error {
	...
	contextPathHash, _ := filepath.Abs(b.Getwd())
	d, err := driver.GetDriver(ctx, "porter-driver", nil, cli.Client(), cli.ConfigFile(), nil, nil, "", nil, nil, contextPathHash)
	if err != nil {
		return err
	}
	...
	opts := map[string]buildx.Options{
		"default": {
			...
			Inputs: buildx.Inputs{
				ContextPath:    b.Getwd(),
				DockerfilePath: build.DOCKER_FILE,
				InStream:       b.In,
			},
	...
	if err != nil {
		return errors.Wrap(err, "could not dockerConsole docker build output")
	}
